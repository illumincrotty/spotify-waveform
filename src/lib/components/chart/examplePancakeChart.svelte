<script>
	import tsv from '$lib/script/carbon';
	import * as Pancake from '$lib/pancake/index';
...
<div class="chart">
	<Pancake.Chart x1={minx} x2={maxx} y1={miny} y2={maxy}>
		...
		<Pancake.Point x={1962} y={390}>
			<div class="text">
				...
				<p>
					...
					<span style="color: var(--chart-theme-7)">—</span>
					<span>trend</span>
				</p>
			</div>
		</Pancake.Point>
		...
		<Pancake.Point x={highest.date} y={highest.avg}>
			<div
				class="annotation"
				style="position: absolute; right: 0.5em; top: -0.5em; white-space: nowrap; line-height: 1;">
...
<style>
	...
	.grid-line.horizontal {
		width: calc(100% + 2em);
		left: -2em;
		border-bottom: 1px dashed rgb(var(--overlay-components) / 0.5);
	}
	.grid-line.vertical {
		height: 100%;
		border-left: 1px dashed rgb(var(--overlay-components) / 0.5);
	}
	.grid-line span {
		position: absolute;
		left: 0;
	...
	.focus {
		...
		left: -5px;
		top: -5px;
		border: 2px solid var(--chart-theme-7);
		border-radius: 50%;
	...
	/* :global(.annotation) {
		color: rgb(var(--overlay-components));
	} */
