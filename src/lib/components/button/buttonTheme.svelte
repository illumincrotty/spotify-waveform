...
<Button
	buttonLabel="Toggle Theme Palette"
	svgLabel="Palette icon"
	{classes}
	on:click={() => {
		// const compStyle = window.getComputedStyle(document.documentElement);
		// const current =
		// 	+compStyle.getPropertyValue('--current-theme-number') ?? 1;
		$themeNumber = ($themeNumber % 4) + 1;
